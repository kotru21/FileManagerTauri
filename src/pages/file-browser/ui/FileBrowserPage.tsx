...
import { ResizableHandle, ResizablePanel, ResizablePanelGroup, TooltipProvider } from "@/shared/ui"
...
export function FileBrowserPage() {
  ...
  const { searchPath, setSearchPath, results: searchResults, clearSearch } = useSearchStore()
                ...
                <div className="max-h-80 overflow-auto divide-y">
                  {searchResults.map((result) => (
                    <SearchResultItem
                      key={result.path}
                      result={result}
                      onSelect={getSearchHandler(result.path, result.is_dir, result.name)}
                    />
                  ))}
                </div>
              </div>
          ...
          <ResizablePanelGroup
            ...
            onLayout={(sizes) => {
              if (sizes.length >= 2) {
                ...
                if (current.sidebarSize !== newSidebar || current.mainPanelSize !== newMain) {
