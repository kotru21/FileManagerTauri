...
import { FileRow, ColumnHeader, InlineEditRow } from "@/entities/file-entry";
...
export function VirtualFileList({
  ...
  const inlineEditIndex = useMemo(() => {
    ...
    if (mode === "new-folder" || mode === "new-file") {
      ...
      if (mode === "new-folder") {
        ...
        // After last folder
        const lastFolderIndex = findLastIndex(
          files,
          (f: FileEntry) => f.is_dir
        );
  ...
  const totalCount =
    files.length + (hasInlineEdit && mode !== "rename" ? 1 : 0);
        ...
        <div
          ...
          {virtualizer.getVirtualItems().map((virtualRow) => {
            ...
            if (
              ...
              const initialName =
                isRename && targetPath ? getBasename(targetPath) : "";
